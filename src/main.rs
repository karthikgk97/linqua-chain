...
use linqua_chain::llm_mod::base_llm::ModelType;
...
async fn main() {
    ...
    let mut ccm = CCMStruct::new(ModelType::OpenAI, true);
    ...
    let mut sqm = SQMStruct::new(ModelType::OpenAI, true);
    sqm.set_temperature(0.2);
    ...
    These are the closest options found that might be benefical for generating the SQL Query: {:?}. 
    UTILIZE THIS and answer the user question.
    The provided closest options are really helpful in answering the user input question", &user_input_question, search_results_vec);
